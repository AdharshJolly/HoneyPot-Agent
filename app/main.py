...
async def handle_message(
    request: MessageRequest, background_tasks: BackgroundTasks, api_key: str = Depends(verify_api_key)
):
    """
    ...
    if session.sessionClosed:
        ...
        last_agent_message = next((msg.text for msg in reversed(session.conversationHistory) if msg.sender == "agent"), None)
        final_reply_for_closed_session = last_agent_message if last_agent_message else "The conversation has ended. Thank you for your messages."
        logger.info(f"Session {session.sessionId} is closed. Returning consistent 200 OK response.")
    ...
    if next_state != session.agentState:
        ...
        session.stateTurnCount[session.agentState] = session.stateTurnCount.get(session.agentState, 0) + 1
    ...
    if session.agentState == "EXIT":
        background_tasks.add_task(callback_dispatcher.check_and_dispatch, session.sessionId)
